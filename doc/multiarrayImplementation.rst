...
(Note that in :abbr:`TMP (template metaprogramming)`, "calling" :class:`~tmptools::numerical_contains`\ ``<V,mpl::_2>`` twice is no waste because the second time no further template instantiations are needed, the compiler will use the ones already instantiated the first time.)

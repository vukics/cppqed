...
/// \briefFileDefault
#ifndef CPPQEDCORE_UTILS_COMMENTING_STREAM_H_INCLUDED
#define CPPQEDCORE_UTILS_COMMENTING_STREAM_H_INCLUDED
...
// Slightly modified by András Vukics for use in C++QED with Boost 1.64 and g++ 7.1.1 in 2017
...
template<typename FiniteStateMachine>
class finite_state_filter_impl : public FiniteStateMachine
{
private:
...
public:
  ...
  BOOST_IOSTREAMS_FSM(CommentingMachine) // Define skip and push.
  typedef CommentingMachine self;
  
  static const int lineBegin = initial_state;
  static const int withinLine = lineBegin + 1;

  static const char commentChar='#';
  
  void pushFirstCharInLine(char c) { push(commentChar); push(' '); push(c); }

  struct is_any_except_EOL {
    template<typename Ch>
    static bool test(Ch c, const std::locale&) { return c!='\n'; }
  };

  typedef boost::mpl::vector<
              row<lineBegin , is<'\n'>         , lineBegin , &self::push               >,
              row<lineBegin , is_any_except_EOL, withinLine, &self::pushFirstCharInLine>,
              row<withinLine, is_any_except_EOL, withinLine, &self::push               >,
              row<withinLine, is<'\n'>         , lineBegin , &self::push               >
          > transition_table;

...
public:
  ...
  const dcomp amp     (double t) const {return         exp(omega1_*t)*a_(0)+        exp(omega2_*t)*a_(1)+               c(t);}
  const dcomp ampDeriv(double t) const {return omega1_*exp(omega1_*t)*a_(0)+omega2_*exp(omega2_*t)*a_(1)+DCOMP_I*omega_*c(t);}
...
public:
  ...
  const dcomp amp     (double t) const {return  exp(-t)*a_(0)+      exp(-t)*a_(1)+               c(t);}
  const dcomp ampDeriv(double t) const {return -exp(-t)*a_(0)-(t-1)*exp(-t)*a_(1)+DCOMP_I*omega_*c(t);}

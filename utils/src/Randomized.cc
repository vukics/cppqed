...
const Randomized::Ptr MakerGSL::operator()(unsigned long seed) const
{
  ...
  private:
    ...
    ostream& writeState(ostream &os) const
    {
    ...
    ostream& writeImplID(ostream &os) const
    {
      return os << implID() << " ";
    }


    istream& readImplID(istream &is) const
    {
      string id;
      char c;
      is >> id;
      if (id!=implID()) throw RNGStateParsingException(string("Wrong implementation ID, expected "+implID()+", found "+id));
    ...
    static const string implID() {return "RandomizedGSL";}
